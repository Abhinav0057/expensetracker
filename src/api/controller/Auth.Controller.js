...
const signupUser=async(req,res,next)=>{
    ...
    try{
        ...
        const newuser= new User({name,email,password:hashedPassword,income:[],expenses:[]})
        try{
            newuser.save();
...
const signinUser = async (req, res, next) => {
    ...
    const token = jwt.sign({ id: existingUser._id, email: existingUser.email }, jwtSecretKey,{ expiresIn: '1w'} );
